...
Now, lets have a look at second example, now TypeScript will tech us something about DOM API. Have a look at the following code, which does not compile (it used to handle React button click event):
...
When we first encountered this problem, we decided there was a bug in TypeScript type definitions for React (those are installed with `npm install --save-dev @types/react`). But later we discovered, that type definitions actually have another property for `SyntheticEvent<T>` (type of `target`), named `currentTarget`, whose type is different:
